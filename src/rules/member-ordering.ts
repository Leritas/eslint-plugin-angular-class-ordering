...
function normalizeOrderEntries(orderRaw: readonly OrderEntry[]): {
    slotKeys: string[];
    patternMatchers: (RegExp | null)[];
} {
    const slotKeys: string[] = [];
    const patternMatchers: (RegExp | null)[] = [];
...
function resolveCategory(
    ...
    patternMatchers: readonly (RegExp | null)[],
    ...
    if (isMethod(member)) {
        ...
        if ('abstract' in member && member.abstract) return 'signature';
        if (member.type === 'TSAbstractMethodDefinition') return 'signature';
    }
