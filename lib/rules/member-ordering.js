...
function resolveCategory(member, importMap, sourceCode, overlayProbes, slotKeys) {
    ...
    const isResolvedImportCall = (node, module, importedName) => {
        if (!isIdentifier(node.callee))
            return false;
        const info = importMap.get(node.callee.name);
        return info?.module === module && (!importedName || info.importedName === importedName);
    };
    const isCoreCall = (node, name) => isResolvedImportCall(node, ANGULAR_CORE, name);
    if (isCallExpr(init)) {
        ...
        if (isCoreCall(init, 'computed'))
            return 'computed';
        if (isResolvedImportCall(init, NGXS_STORE, 'createSelectMap'))
            return 'store-select-map';
        if (isResolvedImportCall(init, NGXS_STORE, 'select'))
    ...
    if (isMethod(member) && (member.kind === 'get' || member.kind === 'set'))
 ...
 */
function calculateRanks(members, categories, orderIndex, unknownPlacement, slotKeysLength, readonlyOrdering) {
    ...
    members.forEach((m, i) => {
        ...
        const readonlyRankAdjustment = readonlyOrdering && isPropertyLike(m) && 'readonly' in m && m.readonly
        ...
        if (idx === undefined) {
            if (unknownPlacement === 'ignore')
                ...
                ranks[i] = slotKeysLength + readonlyRankAdjustment + tieBreaker;
        }
        else {
...
function visitAngularClass(classNode, importMap, ctx) {
    const { context, sourceCode, orderIndex, slotKeys, overlayProbes, slotLabels, unknownPlacement, orderRaw, readonlyOrdering, } = ctx;
    ...
    const { ranks, skipFlags, unknownErrors } = calculateRanks(members, categories, orderIndex, unknownPlacement, slotKeys.length, readonlyOrdering);
    ...
    if (violationIdx !== -1) {
        ...
        if (unknownErrors.length === 0 && unknownPlacement !== 'error') {
            context.report({
                ...baseReport,
                fix: (fixer) => FixerEngine.run(fixer, sourceCode, classNode.body, members, ranks, categories, readonlyOrdering),
            });
        }
        else {
...
const FixerEngine = {
     ...
     */
    run(fixer, sourceCode, classBody, members, ranks, categories, readonlyOrdering) {
        ...
        ordered.forEach((chunk, k) => {
            if (k > 0)
                newText += FixerEngine.calculateGap(ordered[k - 1], chunk, sourceCode, readonlyOrdering);
            newText += chunk.raw;
        });
     ...
     */
    calculateGap(prev, curr, sourceCode, readonlyOrdering) {
