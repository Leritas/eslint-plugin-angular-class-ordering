...
exports.messages = {
    wrongOrder: 'Member "{{member}}" ({{actualCategory}}) must appear before "{{previousMember}}" ({{previousCategory}}). Expected slots: {{expectedOverview}}.',
...
/**
 * Builds parallel arrays of resolved slot keys and optional RegExp matchers for pattern slots.
 */
...
function resolveCategory(member, importMap, sourceCode, patternMatchers, slotKeys) {
    ...
    if (hasDeco(['HostBinding', 'HostListener']))
    ...
    if (isCallExpr(init)) {
        ...
        if (isCoreCall(init, 'hostBinding') || isCoreCall(init, 'hostListener'))
    ...
    if (hasCoreCall(['contentChild', 'contentChildren']))
        return 'content-query-signal';
    if (hasCoreCall(['hostBinding', 'hostListener']))
        return 'host-binding-signal';
    ...
    const patternIdx = patternMatchers.findIndex((rx) => rx?.test(text) ?? false);
    return patternIdx !== -1 ? (slotKeys[patternIdx] ?? null) : null;
}
/**
